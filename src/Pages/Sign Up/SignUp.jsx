...
const SignUp = () => {
    ...
    const handleSignUp= (e)  =>{ 
        ...
        if(!/^(?=.*[A-Z])(?=.*[!@#$%^&*(),.?":{}|<>])(.{6,})$/.test(password)){ 
            Swal.fire({
              title: "Passoword Not Correct!", 
              text: "Minimum six characters,one special character and at least one capital letter needed!", 
              icon: "error"
            }); 
        }else{ 
            if(email){ 
        ...
        <div className="card shrink-0 w-full max-w-sm border border-[#D0D0D0] bg-base-100">
          ...
          <h1 className="text-4xl font-semibold text-center mb-10">
            ...
            <div className="text-center mx-auto">
              ...
              <h1 className="text-lg font-normal text-[#737373]">Already have an account? <NavLink to="/login"><span className="text-lg font-semibold text-[#FF3811]">Sign In</span></NavLink></h1>
            </div>
          </form>
        </div>
      </div>
